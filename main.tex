...
%
\begin{abstract}
% TODO: Resubmit abstract at the end!	
...
\begin{table}
	...
	\begin{tblr}{
		...
		15 & 1 & 325 ms& 32.771 \\
		16 & 1 & 697 ms& 65.539 \\
		17 & 1 & 1.533 ms& 131.075 \\
		20 & 1 & 14.663 ms& 1.048.579 \\
		5 & 3 & 24 ms& 1.027 \\
		5 & 5 & 46 ms& 7.779 \\
		3 & 10 & 25 ms& 1.334 \\
		3 & 20 & 51 ms& 9.264 \\
...
Due to partial order reduction they managed to check previously intractable models and further reduced the analysis time.
...
However, a translation of BPMN to Petri nets is not always possible~\cite{favreDifficultyReplacingInclusive2012}.
...
\begin{table}
	...
	\begin{tblr}{
		...
		% TODO: Run benchmark on fastest machine using WSL. Currently windows.
		e001~\cite{houhouFirstOrderLogicVerification2022} & 4 ms & 31 & Option To Complete, No Dead Activities \\
		e002~\cite{houhouFirstOrderLogicVerification2022} & 4 ms & 31 & Option To Complete, No Dead Activities \\
		e020~\cite{houhouFirstOrderLogicVerification2022} & 13 ms & 2.112 & Option To Complete \\
		credit-scoring-async\footref{footnote:camundaResearch} & 5 ms & 50 & Option To Complete \\
		credit-scoring-sync\footref{footnote:camundaResearch} & 5 ms & 99 & Option To Complete \\
		dispatch-of-goods\footref{footnote:camundaResearch} & 5 ms & 103 & Safeness, Proper Completion\\ 
		recourse\footref{footnote:camundaResearch} & 5 ms & 71 & Option To Complete \\
		self-service-restaurant\footref{footnote:camundaResearch} & 5 ms & 74 & Option To Complete \\
	\end{tblr}
\end{table}

Each model violates one or more soundness property.
...
Thus, our framework is extensible, so one can provide custom resolution strategies.
This is possible due to the \textit{modular} design of the tool, which \textit{decouples} the soundness analysis from reporting violations, visualizing counterexamples, and providing quick fixes.
...
Furthermore, we benefit from Rusts memory-safety and thread-safety guarantees.
...
Our tool is not yet heavily optimized for performance to avoid code complexity and can be extended regarding BPMN features, as well as partial-order reduction to tackle state space explosion.
% Zero-copy? --> Currently, we are copying things but still achieve better performance (Tradeoff with readability)

Our tool can be integrated into existing BPMN modeling tools since it provide model checking as a \textit{web-service} or \textit{command-line application}.
Furthermore, the front-end is \textit{modular} which means one can reuse it as a whole or only selected parts.
In addition, one can extend the front-end in various ways, for example, to support more quick-fixes as discussed in \autoref{subsec:fixable-checking}.
...
For example, our tool finds only 2.112 states for the BPMN model \textsf{e020}, as showin in \autoref{subsec:instantaneous}, compared to the 3.558 in~\cite{houhouFirstOrderLogicVerification2022} or 3.060 in~\cite{krauterFormalizationAnalysisBPMN2023}.
...
Our tools does not support the same depth of BPMN elements as, for example~\cite{krauterFormalizationAnalysisBPMN2023}, but we are confident that we can add support for necessary elements in the future.
