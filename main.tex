...
\begin{figure}[ht]
	...
	\caption{Quick fixes for \textit{Option To Complete}}
	\label{fig:optionToComplete}
\end{figure}

Another way to fix violations is to fing the split in the sequence flow, for example the exclusive gateway in \autoref{fig:optionToComplete}, and make this split a parallelization, see quick fix \textbf{(b)} in \autoref{fig:noDeadActivitiesQuickFix}.
...
\textbf{Resolutions for (2):} We have not implemented any quick fixes for untriggered evenvts yet.
% Explain some ideas such as for message events and signal events. Finding matching throw events can be as complex as desired (similar name, proximity etc).
