...
% TODO: Finaly submission: Add authors, change artifact link, publish all source code.
% \author{
...
% - subsubsection vspace adjust or move to a different custom command
% - Merge the figure 5 and 7.
% - Redo fig. 11 to make it more compact and maybe not side by side.
% - Check other figures and sizes.
% - Dont use itemize to list the reasons for property violations but just put it in the text.
% - Footnote instead of citation for camunda things.
% - Check overflowing sentences etc.
...
% Why useful? Check
Many process models suffer from control-flow errors, such as deadlocks and lack of synchronization~\cite{fahlandAnalysisDemandInstantaneous2011}.
These errors 
% Claims a lot of problems in workflow models, which can be found using soundness checks.

% Solution
In this paper, we describe our new tool, which checks BPMN process models for soundness and safeness~\cite{corradiniClassificationBPMNCollaborations2018}, which entails finding deadlocks, lack of synchronization, and dead activities.
...
Our front-end is based on the popular \textit{bpmn.io} ecosystem, while the soundness checker is implemented in Rust for fast performance, memory-efficiency, and memory safety. % Even blazingly fast performance
...
The tool is able to check models after every change since the soundness checker is \textit{instantaneous}, i.e., takes 500ms or less~\cite{fahlandAnalysisDemandInstantaneous2011}.
...
% Paper structure
In the remainder of the paper, we describe how instantaneous, comprehensive, and fixable soundness checking is achieved in \autoref{sec:soundness}.
...
% Consumability what we call comprehensive is the biggest challenge in fahland
