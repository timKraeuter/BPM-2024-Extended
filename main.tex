...
% Performance using different benchmarks. --> Needs its own paper where everyone can contribute. --> Usually not done but quite important for industrial use. Model checking has to have adequate performance!
\textbf{Soundness checking performance:}
...
Bprove has something interesting? \cite{corradiniFormalApproachAnalysis2021}


\textbf{Animation} previously in \cite{corradiniFormalisingAnimatingMultiple2022,camundaservicesgmbhBpmnjsTokenSimulation2024} but not used for counter examples!

\textbf{Counter example visualization}
fbpmn \cite{houhouFirstOrderLogicVerification2022}

% Summary
% Our tool comprises many ideas implemented before and improves upon them. In addition we have quick fixes and excellent performance with room for improvement (implementation optimization: zero-copy and other tricks? bloom filters etc., partial-order reduction).

\section{Limitations \& Threats to Validity}
% Benchmarking problem: Different machines etc. makes it hard to compare runtime
