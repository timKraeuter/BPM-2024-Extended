...
pdftitle={Instantaneous, Understandable, and Fixable Soundness Checking of Industrial BPMN Models},
...
\begin{document}
%
\title{Instantaneous, Understandable, and Fixable Soundness Checking of Realistic BPMN Models}
%
\titlerunning{Instantaneous, Understandable, and Fixable BPMN Soundness Checking}
...
In this paper, we introduce our new soundness-checking tool that can instantaneously identify errors in BPMN models, make them understandable for modelers, and even suggest fixes to eliminate them automatically.
...
Moreover, the tool displays possible soundness violations directly in the modeler and provides an interactive token-flow animation that illustrates each violation.
Additionally, it provides fixes to resolve the found violations, which, to the best of our knowledge, is not currently available in other tools.
...
% Solution
% Describe the figure and go over all three claims briefly: instantaneous (Definition), understandable (soundness properties are hard to understand for modelers), and  (comparable to quick fixes).
...
% Understandable/Visualization is missing in the figure: instantaneous and fixable are there.
\begin{figure}[ht]
	\centering
...
In remainder of this section we describe how we achieve \textit{instantaneous}, \textit{understandable}, and \textit{fixable} soundness checking of realistic BPMN models.
...
However, reports from researchers indicate that tools operating with five parallel branches show a noticeable delay, with execution times reaching several seconds, suggesting diminishing performance as complexity increases.
% One cannot have definite conclusions and performance was not the focus of other approaches BUT our approach seems to be much faster since it is much more pragmatic. --> Cannot check all properties either
% We invite other researchers to check the same models we have in this paper our come together and make a benchmark for soundness checking.
