...
Subsequently, we progressively add more blocks to the models until we reach 500.
...
An increase in model size leads to a bigger state space that must be analyzed.
...
% Benchmarking problem: Different machines etc. make it hard to compare runtime
% Other researchers have probably not optimized for performance
% Other approaches are implemented in a more formal setting; thus, a comparison may not be apples to apples as we use a pragmatic encoding of semantics directly implemented in Rust.
